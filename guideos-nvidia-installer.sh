...
source "./usr/lib/guideos-nvidia-installer/packages.sh"
...
if packages::is_installed "sudo"; then
    exec gnome-terminal --title="Terminal" -- bash -c "sudo $custom_env_vars ./usr/bin/guideos-nvidia-installer; exec bash"
else
    exec gnome-terminal --title="Terminal" -- bash -c "$custom_env_vars ./usr/bin/guideos-nvidia-installer; exec bash"
fi
