...
tr::add "de_DE" "cudarepo::install_driver.confirm" "Sie sind dabei, den durch das CUDA-Repository bereitgestellten Treiber %1-%2 zu installieren.\n\nMöchten Sie fortfahren?"
tr::add "de_DE" "cudarepo::install_driver.invalid_flavor" "Ein unerwarteter Fehler ist aufgetreten: Driver-Flavor %1 wird vom Skript nicht erkannt."
...
cudarepo::uninstall_cuda_repository() {
    ...
    # Removes the Early KMS APT hook if it exists
    local hook_file="/etc/apt/apt.conf.d/99nvidia-initramfs"
    if [ -f "$hook_file" ]; then
        rm -f "$hook_file"
        log::info "APT-Hook $hook_file entfernt."
    fi
...
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.remove_failure" "Fehler beim Entfernen des Pakets cuda-keyring: Code %1."
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.not_installed" "Das Paket cuda-keyring ist nicht installiert. Wird übersprungen."
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.removed" "Paket cuda-keyring erfolgreich entfernt."
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.removing_sources" "CUDA-Quellendatei wird entfernt..."
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.sources_removed" "CUDA-Quellendatei entfernt: %1"
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.no_sources" "Keine CUDA-Quellendatei zum Entfernen gefunden."
tr::add "de_DE" "cudarepo::uninstall_cuda_repository.sources_remove_failure" "Fehler beim Entfernen der CUDA-Quellendatei: %1"
...
tr::add "de_DE" "cudarepo::unlock_cuda_version.removing_file" "Präferenzdatei wird entfernt: %1..."
tr::add "de_DE" "cudarepo::unlock_cuda_version.unlocked" "%1 Präferenzdateien entfernt, NVIDIA-Treiberversion entsperrt."
tr::add "de_DE" "cudarepo::unlock_cuda_version.not_found" "Präferenzdatei nicht gefunden, keine NVIDIA-Treiberversion fixiert."
tr::add "de_DE" "cudarepo::unlock_cuda_version.remove_failed" "Fehler beim Entfernen der Präferenzdatei: %1"

# ----------------------------------------------------------------------------
# Function: cudarepo::setup_early_kms
# Description: Configures Early KMS and auto-update APT hook to prevent grey
#              screen on boot after NVIDIA driver installation.
# ----------------------------------------------------------------------------
cudarepo::setup_early_kms() {
    log::info "Konfiguriere Early KMS für Nvidia..."

    # 1. Nvidia-Module für Early KMS in initramfs-tools eintragen
    ...
    if [[ -f "$modules_file" ]]; then
        for mod in "${nvidia_modules[@]}"; do
            if ! grep -q "^${mod}\$" "$modules_file"; then
                ...
                log::info " -> Modul '$mod' zu $modules_file hinzugefügt."
            else
                log::info " -> Modul '$mod' bereits vorhanden."
            fi
        done
    fi
...
DPkg::Post-Invoke {
    "if dpkg -l | grep -q firmware-nvidia-gsp; then update-initramfs -u -k all; fi";
};
HOOK_EOF
    log::info " -> APT-Hook unter $hook_file angelegt."

    # 3. DKMS, Initramfs und GRUB aktualisieren
    if command -v dkms &> /dev/null; then
        log::info " -> Baue DKMS-Module..."
        log::capture_cmd dkms autoinstall
    fi

    log::info " -> Aktualisiere Initramfs für alle Kernel..."
    log::capture_cmd update-initramfs -u -k all

    if command -v update-grub &> /dev/null; then
        log::info " -> Aktualisiere GRUB..."
...
tr::add "de_DE" "cudarepo::install_cuda_proprietary.start" "NVIDIA-Treiberinstallation wird gestartet..."
tr::add "de_DE" "cudarepo::install_cuda_proprietary.success" "NVIDIA-Treiber erfolgreich installiert."
...
tr::add "de_DE" "cudarepo::install_cuda_opensource.start" "NVIDIA-Treiberinstallation wird gestartet..."
tr::add "de_DE" "cudarepo::install_cuda_opensource.success" "NVIDIA-Treiber erfolgreich installiert."
